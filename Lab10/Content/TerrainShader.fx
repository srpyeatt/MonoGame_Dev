...
PhongVertexOutput TerrainVertexShader(VertexInput input)
{
    ...
    output.WorldPosition = mul(input.Position, World); // set output WorldPosition
    float4 viewPosition = mul(output.WorldPosition, View);
    output.Position = mul(viewPosition, Projection); // set output position (screen)
    output.UV = input.UV; // set output UV
    return output;
}
...
float4 TerrainPixelShader(PhongVertexOutput input) : COLOR0
{
    ...
    float3 diffuse = max(dot(lightDirection, normal), 0);
