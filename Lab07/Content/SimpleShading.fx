...
float4 PhongPixel(PhongVertexOutput input) : COLOR0
{
    ...
    float3 reflectDirection = -reflect(lightDirection, input.WorldNormal.xyz);
	// Now, compute the lighint components
    float3 diffuse = max(dot(lightDirection, input.WorldNormal.xyz), 0) * tex2D(DiffuseSampler, input.UV).xyz;
    float specular = pow(max(dot(reflectDirection, viewDirection), 0), Shininess);
...
float4 PhongBlinnPixel(PhongVertexOutput input) : COLOR0
{
	...
	// Now, compute the lighint components
    float3 diffuse = max(dot(lightDirection, input.WorldNormal.xyz), 0) * tex2D(DiffuseSampler, input.UV).xyz;
    float3 add = normalize(lightDirection + viewDirection);
    float specular = pow(max(dot(add, input.WorldNormal.xyz), 0), Shininess);
    return float4(AmbientColor + diffuse * DiffuseColor + specular * SpecularColor, 1);
}
...
float4 SchlickPixel(PhongVertexOutput input) : COLOR0
{
	...
	// Now, compute the lighint components
    float3 diffuse = max(dot(lightDirection, input.WorldNormal.xyz), 0) * tex2D(DiffuseSampler, input.UV).xyz;
    float3 t = dot(reflectDirection, viewDirection);
    float3 s = normalize(t / ((Shininess + t) - (t * Shininess)));
    float specular = pow(t, s);
    return float4(AmbientColor + diffuse * DiffuseColor + specular * SpecularColor, 1);
}
