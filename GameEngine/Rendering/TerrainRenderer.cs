...
namespace CPI311.GameEngine.Rendering
{
    public class TerrainRenderer: Component
    {
            ...
            for (int r = 0; r < rows; r++)
                for (int c = 0; c < cols; c++)
                    Vertices[r * cols + c] = new VertexPositionTexture(
                        //offset + new Vector3(c * stepX, 0, r * stepZ),  // xyz
                        offset + 
                        new Vector3(
                            c * stepX, 
                            GetHeight(new Vector2(c / res.X, r / res.Y)), 
                            r * stepZ),
                        new Vector2(c / res.X, r / res.Y));             // uv
        ...
        public float GetHeight(Vector2 tex) // tex is UV [0, 1]
        {
        ...
        public float GetAltitude(Vector3 position)
        {
            ...
            if (position.X > -size.X / 2 && position.X < size.X / 2 &&
                ...
                return 
                    GetHeight( new Vector2 ( 
                        (position.X + size.X/2)/size.X, (position.Z + size.Y/2)/size.Y ))
